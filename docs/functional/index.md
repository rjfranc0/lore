# Functional

What lore does, from the perspective of someone running the CLI — the
symlink-based config model, and the multi-account Claude wiring built on
top of it.

- [agent-config](agent-config.md) — the original surface: `~/.agents/`,
  `AGENTS.md`, skill install/remove, behavior add/remove, `sync`, `list`,
  `update`.
- [accounts](accounts.md) — the `lore.toml` config file, `lore init`'s
  bootstrap/migration behavior, and the full account lifecycle (`init
  --account`, `accounts list/remove/sync`).
