...
- No per-account **behavior update/sync**: `lore update`/`lore sync` still
  only ever operate on the shared `~/.agents/` tree — a scoped behavior
  added via `behavior add --account <name>` has no equivalent re-link or
  reconciliation command yet, it's only reachable via `add`/`remove`
  themselves. See [@/functional/accounts.md] for what else varies per
  account.
